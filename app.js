...
const STORE_KEY="gameIndexProgressV2";
...
function masteryCard(type,r,i){
  ...
  return `<div class="mastery-card">
    ...
    <div class="controls">
      <button onclick="toggleObtained('${type}','${esc(name)}')">${obt?"☑ Obtained":"☐ Obtained"}</button>
...
function collectionCategory(type){
  ...
  app.innerHTML=header(g[1],`${rows.length} items`,"Collections","collections")+`<main><div class="searchbox"><input placeholder="Search..." oninput="filterCards(this.value,'collectionList')"></div><div id="collectionList" class="listcards">${rows.map(r=>{
    ...
    return `<div class="mastery-card"><div class="row between"><h3>${esc(n)}</h3><button onclick="toggleObtained('${type}','${esc(n)}');collectionCategory('${type}')">${obt?"☑ Obtained":"☐ Obtained"}</button></div><div class="meta">${details(type,r)}</div></div>`;
  }).join("")}</div></main>`;
}
